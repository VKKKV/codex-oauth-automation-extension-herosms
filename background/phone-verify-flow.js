(function attachPhoneVerifyFlow(root, factory) {
  ...
  function createPhoneVerifyFlow(deps = {}) {
    ...
    async function tryReuseLastActivation(apiKey, country, service) {
      ...
      await addLog(
        `步骤 8：检测到上次 HeroSMS 号码 +${last.phone}（activationId=${last.activationId}），正在验证是否仍可复用...`,
        'info'
      );
      let status;
      try {
    ...
    async function run(step, state, { url } = {}) {
      ...
      try {
        ...
        if (firstRound.ok) {
          ...
          // Instead of Round 2, we clear the last activation and throw a special error to restart from Step 7
          if (typeof clearLastActivation === 'function') {
            try { await clearLastActivation(); } catch (_) {}
          }
          await safeCancelActivation(config.apiKey, activationId);
          throw new Error(`STEP8_RESTART_STEP7::步骤 8：HeroSMS 首轮 ${Math.round(POLL_TIMEOUT_MS / 1000)} 秒内未收到短信，按用户要求放弃该号码并从步骤 7 重新开始以获取新号码。`);
        }
